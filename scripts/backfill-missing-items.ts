...
const USER_AGENT =
  'ge-skiller-import/1.0 (contact: your-contact-or-repo-url-here)';
...
async function fetchWikiPageContent(
  ...
  if (!res.ok) {
    throw new Error(
      `Wiki parse error for "${pageTitle}": ${res.status} ${res.statusText}`
    );
  }
...
function mapInfoboxToOsrsboxFields(
  ...
  const releaseVal =
    get('release', 'released', 'release date') ||
    get('release1', 'introduced');
...
function applyIfMissing(
  base: OsrsboxItemInput,
  extra: Partial<OsrsboxItemInput>
): OsrsboxItemInput {
  const out = { ...base };
...
function buildSkeletonFromMapping(m: MappingItem): OsrsboxItemInput {
  ...
  return {
    ...
    name: m.name,
    wiki_name: m.name,
    wiki_url: `https://oldschool.runescape.wiki/w/${encodeURIComponent(
      m.name.replace(/ /g, '_')
    )}`,
...
async function fetchWikiFullHtml(pageTitle: string): Promise<string> {
  const url = `https://oldschool.runescape.wiki/w/${encodeURIComponent(
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch full HTML for "${pageTitle}": ${res.status} ${res.statusText}`
    );
  }
    ...
    try {
      if (reboxedId != null) {
        ...
        if (!mappingCandidates || mappingCandidates.length === 0) {
          ...
          console.warn(
            `[WARN] No osrsreboxed or mapping entry found for "${rawName}" (key: "${lookupKey}")`
          );
          continue;
        }
      ...
      // 3. Enrich with OSRS Wiki infobox (wikitext + HTML)
        ...
        console.warn(
            `[WARN] Wiki API parse failed for "${pageTitle}": ${apiErr?.message || apiErr}`
        );
        }

        // 🔁 Backup: if API produced nothing useful, fall back to full HTML crawl
        if (
        Object.keys(wtBox).length === 0 &&
        Object.keys(htmlBox).length === 0
        ) {
        try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            console.warn(
            `[WARN] Full HTML crawl failed for "${pageTitle}": ${htmlErr?.message || htmlErr}`
            );
        }
        }
...
baseItem = applyIfMissing(baseItem, wikiFields);
      ...
      await OsrsboxItemModel.updateOne(
        { id: finalItem.id },
        { $set: finalItem },
        { upsert: true }
      );
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      console.error(
        `[ERROR] Failed to backfill "${rawName}":`,
        err?.message ?? err
      );

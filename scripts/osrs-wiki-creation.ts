...
function buildMethodFromCaptionTables(
  ...
  for (const { captionText, $table } of captionTables) {
    ...
    if (/requirement/.test(captionLower)) {
      ...
      const { materials: mats, products: inlineProducts } =
        parseMaterialsAndInlineProducts($, $table, pageTitle);
      materials.push(...mats);
...
async function getCreationSectionIndex(title: string): Promise<number | null> {
  ...
  const creationSection = sections.find(
    (s) => s.line.toLowerCase().trim() === "creation"
  );
 ...
 */
async function getCreationSectionHtml(
  title: string,
  sectionIndex: number
): Promise<string | null> {
  const data = await wikiApi({
  ...
  if (typeof data?.parse?.text === "string") {
    ...
    data?.parse?.text &&
    typeof (data.parse.text as any)["*"] === "string"
  ) {
...
function parseMaterialsAndInlineProducts(
  ...
  $rows.slice(1).each((_, row) => {
    ...
    if (firstCell.is("th")) {
      ...
      if (
        /total cost|profit after ge tax|profit/.test(headerText) &&
        headerText.length > 0
      ) {
        return;
      }
    ...
    const qtyText = $qtyCell.text().replace(/[^0-9]/g, "").trim();
 ...
 */
export function parseCreationSectionToMethods(
  pageTitle: string,
  creationHtml: string
): CreationMethod[] {
  ...
  if ($tabs.length > 0) {
    ...
    $tabs.each((_, el) => {
      ...
      const methodName =
        $tab.attr("data-title") ||
        $tab.attr("data-hash") ||
        undefined; // e.g. "Needle", "Costume needle"

      const captionTables = extractCaptionTables($, $tab);
      if (captionTables.length === 0) return;

      const method = buildMethodFromCaptionTables(
        $,
        pageTitle,
        captionTables,
        methodName
